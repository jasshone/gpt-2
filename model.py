...
class Transformer(nn.Module):
  def __init__(self, B, seq_len, embedding_dim, dk, dv, n_heads, N_blocks, vocab_size):
    ...
    self.decoder = decoder(B, seq_len, embedding_dim, dk, dv, n_heads, N_blocks, vocab_size)
  ...
  def get_pos_encoding(self, x):
    ...
    return self.pos_encoding.to(device)

    torch.sin(x)
  def forward(self, x):
    #print(self.lookup(x).shape, self.get_pos_encoding(x).shape)
    x = self.lookup(x) + self.get_pos_encoding(x).to(device)
